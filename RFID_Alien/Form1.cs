    ...
    public partial class Form1 : Form
    {
            ...
            try
            {
                if (data != null && data.Length > 0 &&data.IndexOf("No Tags")==-1)
                {
                    ...
                    {*/
                        ...
                        WriteData(data, DateTime.Now.ToString("yyyyMMddHHmmssfff") + "Tz" + convertTimeZone(TimeZone.CurrentTimeZone.GetUtcOffset(DateTime.Now).ToString()));
                        WriteText(data + "\n");
                   // }
                }
            }
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    int count = AlienUtils.ParseTagList(reader.TagList,out tagInfo);
                    if(count>0)
                    {
                        foreach(TagInfo info in tagInfo)
                        {
                            WriteData(info.TagID,DateTime.UtcNow.ToString());
